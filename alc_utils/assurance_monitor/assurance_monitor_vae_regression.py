...
"""This file defines the implemenation of AssuranceMonitor based on VAE using only inputs for regression."""
from __future__ import print_function

from assurance_monitor import AssuranceMonitor
import numpy as np
from deep_vae_regression import deepVAE
from icad_vae_regression import ICAD
from assurance_monitor_svdd_vae.martingales import RPM, SMM, PIM
from scipy import stats
import os
from detector import StatefulDetector

ASSURANCE_MONITOR_NAME = "AssuranceMonitorVAERegression"


class AssuranceMonitorVAERegression(AssuranceMonitor):
    _variant = "vae_regression"

    def __init__(self):
        super(AssuranceMonitorVAERegression, self).__init__()
        self.epsilon = 0.75
        self.window_size = 10
        self.rpm = None
        self.pim = None
    ...
    def save(self, save_dir, data_formatter_path=None, lec_storage_metadata=None, make_unique_subdir=False):
        ...
        _save_dir = super(AssuranceMonitorVAERegression, self).save(_save_dir,
                                                                    data_formatter_path,
